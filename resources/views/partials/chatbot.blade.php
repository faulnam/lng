...
<div x-data="nusantaraLngChatbot()" x-init="init()" class="relative font-sans text-[#111111]" x-cloak>
    ...
    <div x-show="showTeaser && !isOpen"
         ...
         class="fixed bottom-22 right-4 sm:bottom-24 sm:right-6 z-50 max-w-[280px] sm:max-w-[320px] bg-white text-black p-3.5 sm:p-4 shadow-2xl border border-neutral-300 flex items-start gap-3 select-none">
    ...
    <button @click="toggleChat()"
            class="fixed bottom-5 right-5 sm:bottom-6 sm:right-6 z-50 w-14 h-14 bg-black text-white border-2 border-black hover:bg-white hover:text-black shadow-2xl flex items-center justify-center transition-all duration-300 group focus:outline-none"
            :class="{ 'bg-white text-black': isOpen }"
            aria-label="Asisten AI Nusantara LNG">
        
        <!-- Chat Icon (Closed) -->
        <svg x-show="!isOpen" class="w-6 h-6 transition-transform duration-200 group-hover:scale-110" fill="none" stroke="currentColor" viewBox="0 0 24 24">
            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M8 12h.01M12 12h.01M16 12h.01M21 12c0 4.418-4.03 8-9 8a9.863 9.863 0 01-4.255-.949L3 20l1.395-3.72C3.512 15.042 3 13.574 3 12c0-4.418 4.03-8 9-8s9 3.582 9 8z"></path>
        </svg>

        <!-- Close Icon (Open) -->
        <svg x-show="isOpen" class="w-6 h-6 transition-transform duration-200 group-hover:rotate-90" fill="none" stroke="currentColor" viewBox="0 0 24 24">
