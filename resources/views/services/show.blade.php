...
@section('content')
    ...
    <section class="relative bg-neutral-900 text-white pt-36 pb-20 md:pt-48 md:pb-28 overflow-hidden">
        <div class="absolute inset-0 bg-cover bg-center opacity-60 scale-105 transform transition-transform duration-1000" 
             style="background-image: url('{{ app_image($service->image, 'images/lng/carrier.jpg') }}');">
        </div>
        ...
        <div class="relative z-10 max-w-4xl mx-auto px-6 text-center space-y-4">
            <div class="text-[9px] sm:text-[10px] md:text-xs uppercase tracking-wider md:tracking-widest2 text-gray-300 font-medium leading-relaxed">
                <a href="{{ url('/services') }}" class="hover:underline">Products &amp; Solutions</a> &bull; <span class="text-white">{{ $service->title }}</span>
            </div>
